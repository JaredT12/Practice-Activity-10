...
function getCards(){
    if(localStarge.getItem('cards') && localStarge.getItem('cards') != '[]'){
        return JSON.parse(localStarge.getItem('cards') )
    }else{
        return intitial_cards
    }
}
function addNewCard(event){
    event.preventDefault()
    let t = document.querySelector("#place").value
    let d = document.querySelector("#descrition").value
    let p = document.querySelector("#poaster").value
    let cards = getCards()
    if(t && d && p){
